        ...
        <!-- <div class="col-md-8 col-sm-6  ">
            <div class="x_panel">
                ...
                <div class="x_content2">
                    <div id="graph_line" style="width:100%; height:350px;"></div>
                </div>
            </div>
        </div>
                    ...
                    <div class="x_title">
                        ...
                        <div class="filter d-flex">
                            <div class="col-md-5 col-sm-5">
                                <select id="category_id" class="form-control " style="width: unset; appearance: auto; border-radius: 3px;">
                                    <option value=0>All</option>
                                    <?php
                                    $categories = getAllByStatus('categories', 1);
                            ...
                            <div id="reportrange" class="pull-right col-md-7 col-sm-7 " style="background: #fff; cursor: pointer; padding: 5px 10px; border: 1px solid #ccc; border-radius: 3px;">
...
<script>
    ...
    function init_flot_chart(data = [0]) {
        ...
        if (data == 0) {
            ...
            data.forEach((item, index) => {
                ...
                if (index > 0 && new Date(data[index - 1].create_at).getTime() == timestamp) {
                    let lastItem = chart_plot_02_data[chart_plot_02_data.length - 1];
                    lastItem[1] += parseInt(item.netSales);
                } else {
                    chart_plot_02_data.push([timestamp, parseInt(item.netSales)]);
                }
            });
            if (chart_plot_02_data.length == 1) {
                chart_plot_02_data.push([new Date().getTime(), chart_plot_02_data[0][1]]);
            }
            console.table(chart_plot_02_data);
        }
    ...
    function loadData() {
        ...
        $.ajax({
            method: "POST",
            url: "../../admin/controllers/statistic.php",
            data: {
                statistic_btn: true,
            ...
            success: function(response) {
                ...
                if (data.length == 0) {
                    $("#products_table tbody").html("<tr><td colspan='7'>No product to show.</td></tr>");
                    return;
                }
                data.forEach((item, index) => {
                    html += `
                    <tr>
                        ...
                        <td>${item.id}</td>
                        <td>${item.name}</td>
                        <td><img src="../../upload/images/${item.img}" width="50px" height="50px" alt="${item.name}"></td>
                        <td>$${item.netSales}</td>
...
<!-- <script>
    function init_echarts() {
        ...
        if ($("#echart_pie").length) {
            ...
            let dataName = [
                <?php
                $orderStatus = getAll("order_status");
            ...
            let countOfOrderStatus = [
                <?php
                $orderStatus = getAll("order_status");
                foreach ($orderStatus as $item) {
                    $data = mysqli_fetch_array(findCountOrderByOrderStatus_Id($item['id']));
                ?> {

                        value: <?php echo $data['totalOrder'] ?>,
    ...
    function init_morris_charts() {
        ...
        if ($("#graph_line").length) {
            let totalPrice = [
                <?php
                for ($i = 1; $i <= 12; $i++) {
                    $data = mysqli_fetch_array(findTotalPriceAtMonth($i));
                ?> {

                        month: "2024-<?php echo $i ?>",
                        value: <?php echo $data['totalPrice'] == null ? 0 : $data['totalPrice'] ?>,
                    },
                <?php
                }
                ?>
